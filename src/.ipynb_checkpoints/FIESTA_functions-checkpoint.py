...
# 	ξ 		= np.fft.fftfreq(n, d=spacing)	
# 	A 	= np.abs(fourier)
# 	ϕ 		= np.angle(fourier)
# 	return [fourier, A, ϕ, ξ]
...
def FIESTA(V_grid, CCF, eCCF, template=[], SNR=2, k_max=None, PRINT=False):
	'''
	...
	if ~np.all(eCCF == 0):
		...
		# If no noise is present in the input spectra, only use the first 10 positive frequency modes
		# if np.all((eCCF == 0)):
		# 	k_max = 10
