...
#--------------------------------------------------------------------
CCF 	= 1 - CCF
df, shift_spectrum, err_shift_spectrum, power_spectrum, err_power_spectrum, RV_gauss = FIESTA(v_grid, CCF, σCCF, k_max = 10)
shift_spectrum 		*= 1000
...
time_series(x=(bjd-bjd[0])*24*60, y=shift_spectrum[0:9,:].T, dy=err_shift_spectrum[0:9,:].T, N=None,
				title='$\Delta RV_k$ time series',
				file_name='FIESTA_shift_time_series.png')

time_series(x=(bjd-bjd[0])*24*60, y=power_spectrum[0:9,:].T, dy=err_power_spectrum[0:9,:].T, N=None,
...
plt.gcf().subplots_adjust(bottom=0.2)
plt.errorbar((bjd-bjd[0])*24*60, rv-np.mean(rv), σrv, marker='.', color='black', ls='none', alpha=alpha)
plt.title('NEID solar RV (1 June 2021)')
plt.xlabel('T [min]')
plt.ylabel('RV [m/s]')
plt.savefig('NEID_solar_RV.png')
plt.show()
